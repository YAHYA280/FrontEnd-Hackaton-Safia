  ...
  const { nodes, edges } = useCallback(() => {
    ...
    voyageData.villes.forEach((ville, villeIndex) => {
      ...
      if (villeIndex < voyageData.villes.length - 1 && graphConfig.showTransportNodes) {
        ...
        // Chercher le dernier emplacement de la ville de départ (peut être dans n'importe quel jour)
        const lastJour = ville.jours[ville.jours.length - 1];
        let lastEmplacement = null;
        if (lastJour.emplacements.length > 0) {
          lastEmplacement = lastJour.emplacements[lastJour.emplacements.length - 1];
        } else {
          // Chercher dans les jours précédents
          for (let i = ville.jours.length - 1; i >= 0; i--) {
            if (ville.jours[i].emplacements.length > 0) {
              lastEmplacement = ville.jours[i].emplacements[ville.jours[i].emplacements.length - 1];
              break;
            }
          }
        }
        
        // Si on a trouvé un emplacement, créer l'edge vers le transport
        if (lastEmplacement) {
          edges.push({
            id: `e${edgeId++}`,
            source: lastEmplacement.id_emplacement,
            target: transportNodeId,
            style: { stroke: "rgb(180 83 9)", strokeWidth: 4, strokeDasharray: "12 8" },
            type: "smoothstep",
            animated: true,
            label: "🚗 Transport",
          });
        ...
        // Chercher le premier emplacement de la ville d'arrivée
        const firstJour = nextVille.jours[0];
        let firstEmplacement = null;
        if (firstJour.emplacements.length > 0) {
          firstEmplacement = firstJour.emplacements[0];
        } else {
          // Chercher dans les jours suivants
          for (let i = 0; i < nextVille.jours.length; i++) {
            if (nextVille.jours[i].emplacements.length > 0) {
              firstEmplacement = nextVille.jours[i].emplacements[0];
              break;
            }
          }
        }
        
        // Si on a trouvé un emplacement, créer l'edge depuis le transport
        if (firstEmplacement) {
          edges.push({
            id: `e${edgeId++}`,
